...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const pageData = dict.localSeo.websCorporativesReus;
  return {
    title: `${pageData.heroTitle} - Nexa Digital`,
    description: pageData.heroSubtitle,
  };
}

export default async function WebsCorporativesReusPage({ params }: { params: { lang: string } }) {
  const { lang } = await params;
  const dict = await getDictionary(lang as "ca" | "es");
  const pageData = dict.localSeo.websCorporativesReus;
      ...
      <main className="flex-1 bg-white">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href={`/${lang}/#contacte`} className="w-full sm:w-auto px-8 py-4 bg-brand-accent hover:bg-brand-secondary text-brand-primary font-bold rounded-xl transition-all hover:scale-105 active:scale-95 text-center shadow-[0_0_20px_rgba(79,209,197,0.3)]">
                  {dict.servicePage.ctaForm}
                </a>
                <a href="tel:+34699368895" className="w-full sm:w-auto px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2">
                  <Phone size={18} className="text-brand-accent" /> {dict.servicePage.ctaPhone}
                </a>
        ...
        <section className="py-20 bg-[#060A16] text-white text-center relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 max-w-3xl relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href={`/${lang}/#contacte`} className="w-full sm:w-auto px-8 py-4 bg-brand-accent hover:bg-brand-secondary text-brand-primary font-bold rounded-xl transition-all hover:scale-105 active:scale-95 text-center shadow-[0_0_20px_rgba(79,209,197,0.2)]">
                {dict.servicePage.ctaForm}
              </a>
              <a href="tel:+34699368895" className="w-full sm:w-auto px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2">
                <Phone size={18} className="text-brand-accent" /> {dict.servicePage.ctaPhone}
