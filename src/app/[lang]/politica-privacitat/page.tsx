...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  return {
    title: `${dict.privacyPage.title} - Nexa Digital`,
    description: dict.privacyPage.title,
  };
}

export default async function PoliticaPrivacitatPage({ params }: { params: { lang: string } }) {
  const { lang } = await params;
  const dict = await getDictionary(lang as "ca" | "es");
  ...
  return (
    <div className="flex flex-col min-h-screen bg-brand-background text-brand-text">
      ...
      <main className="flex-1 container mx-auto px-4 pt-32 pb-20 md:pt-48 md:pb-32 max-w-3xl">
        ...
        <p className="text-brand-text/60 mb-12 font-medium">{dict.privacyPage.lastUpdated}</p>

        <div className="space-y-10">
          {dict.privacyPage.content.map((section: any, idx: number) => (
            <div key={idx}>
